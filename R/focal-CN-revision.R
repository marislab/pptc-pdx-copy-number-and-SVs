...
# Function: Generate Focal copy number matrix
# Date: 07/09/2019
##########################################################################
...
load("~/Box Sync/PPTC-genomics-collaboration/Pedcbio-upload/2019-02-14-pptc-STAR_cufflinks_hg19_matrix_244.rda", verbose = T) 
###load gistic output
...
clin <- read.delim(paste0(pptc.folder, "Data/clinical/2019-07-05-clin.txt"), as.is = T, header = T)
...
###create empty ATRX row and rbind to add deletions
atrx <- matrix(c(""), 1, ncol(genecn2))
rownames(atrx) <- "ATRX"
names(atrx) <- names(genecn2)
genecn2 <- bind_rows(genecn2, atrx)

###add ATRX deletions due to WES bam coverage analysis
if("COG-N-625x" %in% colnames(genecn2)) {
  genecn2["ATRX","COG-N-625x"] <- "Hem_Deletion"
}
if("COG-N-625x" %in% colnames(genecn2)) {
  genecn2["ATRX","ETP-1"] <- "Hem_Deletion"
}
if("OS-1" %in% colnames(genecn2)) {
  genecn2["ATRX","OS-1"] <- "Hom_Deletion"
}
if("OS-45-TSX-pr1" %in% colnames(genecn2)) {
  genecn2["ATRX","OS-45-TSX-pr1"] <- "Hom_Deletion"
}
if("OS-60" %in% colnames(genecn2)) {
  genecn2["ATRX","OS-60"] <- "Hom_Deletion"
}
